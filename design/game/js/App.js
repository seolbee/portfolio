class App{
    ...
    getOrCreateEnemy(data){
        ...
        enemy.reset(data.x, data.y, data.w, data.h, data.s, data.v, data.imgs, data.b, data.hp, data.bullet, data.name);
    }
        ...
        this.bulletList.filter(x=>x.active).forEach(b=>{
            if(!b.isEnemy){
                this.enemyList.filter(x=>x.active).forEach(e=>{
                    if(e.checkCollection(b.x, b.y, b.r)){
                        e.setDemage(b.demage);
                        b.active = false;
                    }
                })
            } else{
                if(this.player.checkCollection(b.x, b.y, b.r)){
    ...
    GameClear(){
        ...
        setTimeout(this.intro.bind(this), 2000);
    }
